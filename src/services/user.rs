...
use crate::other::{
    structs::{Chpwd, Login, User, Username},
    utility::{checkmail_fn, checkname_fn, get_conn_fn, logged_uname_fn},
};
...
async fn login(valuser: web::Json<Login>) -> impl Responder {
    ...
    match conn
        .exec_first(
            "SELECT uid, uusername, upassword FROM users WHERE uusername =:uname",
            params! { "uname" => &valuser.username},
        )
        .map(|row| {
            row.map(|(uid, uusername, upassword)| Login {
                id: uid,
                username: uusername,
                password: upassword,
            })
        ...
        Ok(res) => {
            if !res.is_none()
                && bcrypt::verify(valuser.password.clone(), &res.as_ref().unwrap().password)
            {
                match conn.exec_drop(
                    "UPDATE users SET ulogged = 1 WHERE uusername=:uname",
                    params! { "uname" => &valuser.username},
                ) {
                    Ok(_) => {
...
async fn logout(username: web::Json<Username>) -> impl Responder {
    ...
    match conn.exec_drop(
        "UPDATE users SET ulogged = 0 WHERE uusername=:uname",
        params! { "uname" => &username.username},
    ) {
        Ok(_) => {
...
async fn add(user: web::Json<User>) -> impl Responder {
    ...
    match checkmail_fn(user.email.clone()) {
        Ok(res) => {
            if res {
                ...
                return HttpResponse::BadRequest()
                    .json(json!({ "message":"Email already taken!" }));
            }
        }
        Err(err) => {
    ...
    match conn.exec_drop(
        "INSERT INTO users(uusername, uemail, upassword) VALUES (?, ?, ?)",
        (
            &user.username,
            &user.email,
            bcrypt::hash(&user.password).unwrap(),
        ),
    ) {
        Ok(_) => {
            match conn
                ...
                Ok(res) => {
                    if !res.is_none() {
                        ...
                        return HttpResponse::BadRequest().json(
                            json!({ "message":"User does not exist? Database magic broke" }),
                        );
                    }
                }
                Err(err) => {
...
async fn chpwd(chpwd: web::Json<Chpwd>) -> impl Responder {
    ...
    match conn
        ...
        Ok(res) => {
            if !res.is_none()
                && bcrypt::verify(chpwd.oldpwd.clone(), &res.as_ref().unwrap().password)
            {
                ...
                return HttpResponse::BadRequest()
                    .json(json!({ "message":"Old password invalid!" }));
            }
        }
        Err(err) => {
            error!("database threw error: {err}");
