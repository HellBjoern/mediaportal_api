...
use crate::{other::structs::Config, CONFIG, SQL};
...
pub fn checkname_fn(username: String) -> Result<bool, String> {
    ...
    match conn
        .exec_first(
            "SELECT uusername FROM users WHERE uusername =:uname",
...
pub fn logged_uname_fn(username: String) -> Result<bool, String> {
    ...
    match conn
        .exec_first(
            "SELECT ulogged FROM users WHERE uusername =:uname",
            params! { "uname" => username },
        )
        .map(|row: Option<bool>| row.unwrap())
    {
...
pub fn logged_uid_fn(uid: i32) -> Result<bool, String> {
    ...
    match conn
        .exec_first(
            "SELECT ulogged FROM users WHERE uid =:uid",
            params! { "uid" => uid },
        )
        .map(|row: Option<bool>| row.unwrap())
    {
...
pub fn get_conf() -> Config {
    match std::fs::read_to_string(crate::CONFPATH) {
        Ok(content) => match toml::from_str(&content) {
            ...
            Err(err) => warn!(
                "failed to parse config file, falling back to default; reason: {err}, path: {}",
                Path::new(crate::CONFPATH).display()
            ),
        },
        Err(err) => warn!(
            "failed to load config file, falling back to default; reason: {err}, tried loading {}",
            Path::new(crate::CONFPATH).display()
        ),
    };
    return Config {
...
pub fn yt_dl(uri: String, format: i32) -> Result<(Vec<String>, String), String> {
    ...
    let output = CheckedCommand::new("yt-dlp")
        .args(&args)
        .arg(&uri)
        .arg(saveloc)
        .spawn()
...
pub fn ffmpeg(format: i32, infile: String, mut fname: String) -> Result<String, String> {
    ...
    let output = CheckedCommand::new("ffmpeg")
        .args(input)
        .args(outargs)
        .spawn()
        .expect("failed to execute process")
